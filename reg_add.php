    ...
    <div class="container">
		...
		$get_term=2;
			// $result = mysql_query("select `course no.`,`cid` from course where term='$get_term' ORDER BY `cid` DESC");
			 ...
			 $result = mysql_query("SELECT `subject_id`,`code`FROM `subject`WHERE `code` NOT IN (SELECT `code` FROM `subject` AS sb JOIN `registration` AS rg ON sb.`code` = rg.`subject_id` WHERE rg.`student_no`='$session_id' AND sb.`year`<='$get_year') and `year`<='$get_year' and `term`='$term'");
