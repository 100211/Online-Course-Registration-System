    ...
    <div class="container">
		<div class="margin-top">
			...
			$tgpa=0;
			 ...
			 $var = mysql_query("SELECT DISTINCT `code`,`title`,sb.`unit`,rs.`Grade_point`,rs.`Letter_Grade`,rs.`term`,rs.`year` FROM `subject` AS sb JOIN `result` AS rs ON sb.`code` = rs.`sub_code` WHERE `student_id` = '$session_id' and rs.`Grade_Point`> '0' ");
            ...
            -->
				          <table cellpadding="0" cellspacing="0" border="0" class="table  table-bordered" id="example">                      
                                ...
                                <tbody>
                                  ...
                                  <?php  $user_query=mysql_query("SELECT DISTINCT `code`,`title`,sb.`unit`,rs.`Grade_point`,rs.`Letter_Grade`,rs.`term`,rs.`year` FROM `subject` AS sb JOIN `result` AS rs ON sb.`code` = rs.`sub_code` WHERE `student_id` = '$session_id' and rs.`Grade_Point`> '0'")or die(mysql_error());
