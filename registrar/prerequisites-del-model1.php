...
<?php
	...
	mysql_query("DELETE FROM pre_req1 WHERE req_sub=$sel_id LIMIT 1"); //Delete Row


	
	$sql = 'SELECT distinct(subject_id), `code` as course_no, `title` as course_title, unit FROM subject as t1 inner join pre_req1 as t2 on t1.subject_id=t2.req_sub where t2.select_sub = '.$req_id.'';
	$var = mysql_query($sql);
