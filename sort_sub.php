    ...
    <div class="container">
		<div class="margin-top">
           ...
           <!--         <?php     $total_credit = 0;
			...
			if($status==1){
			 $var = mysql_query("SELECT DISTINCT `code`,`title`,sb.`unit`,rs.`Grade_point`,rs.`Letter_Grade`,rs.`term`,rs.`year` FROM `subject` AS sb JOIN `result` AS rs ON sb.`code` = rs.`sub_code` WHERE `student_id` = '$session_id' and rs.`Grade_Point`> '0' and rs.term like '%$semester%' and rs.year like '%$school_year%' ");
			 }
			 
			if($status==2){
			 $var = mysql_query("SELECT `code`,`title`,`unit`,`year`,`term`  from `subject`  where `code` not in ( SELECT DISTINCT `code`  FROM `subject` AS sb  JOIN `result` AS rs ON sb.`code` = rs.`sub_code` WHERE `student_id` = '$session_id' and rs.`Grade_Point`> '0') and term like '%$semester%' and year like '%$school_year%' ");
			 } 
            ...
            -->
		          <table cellpadding="0" cellspacing="0" border="0" class="table  table-bordered" id="example">                      
                        ...
                        <tbody>
                          	...
                          	<?php  
if($status==1){
                          	$user_query = mysql_query("SELECT DISTINCT `code`,`title`,sb.`unit`,rs.`Grade_point`,rs.`Letter_Grade`,rs.`term`,rs.`year` FROM `subject` AS sb JOIN `result` AS rs ON sb.`code` = rs.`sub_code` WHERE `student_id` = '$session_id' and rs.`Grade_Point`> '0' and rs.term like '%$semester%' and rs.year like '%$school_year%' ")or die(mysql_error());
							}
							
							
				if($status==2){
				$user_query = mysql_query("SELECT `code`,`title`,`unit`,`year`,`term`  from `subject`  where `code` not in ( SELECT DISTINCT `code`  FROM `subject` AS sb  JOIN `result` AS rs ON sb.`code` = rs.`sub_code` WHERE `student_id` = '$session_id' and rs.`Grade_Point`> '0') and term like '%$semester%' and year like '%$school_year%' ");
			 } 			
	if($status==3){
			 $user_query = mysql_query("SELECT DISTINCT `code`,`title`,sb.`unit`,rs.`Grade_point`,rs.`Letter_Grade`,rs.`term`,rs.`year` FROM `subject` AS sb JOIN `result` AS rs ON sb.`code` = rs.`sub_code` WHERE `student_id` = '$session_id' and rs.`Grade_Point` = '0' and rs.term like '%$semester%' and rs.year like '%$school_year%' ");
			 }
